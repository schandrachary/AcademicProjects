...
{
	cout << "Usage: " << pname  << " [Dataset File]" << " [Result File]" << " [ProblemSize: 128|256|512|1024]" << endl;
}
...
//	const unsigned int file_limit = (unsigned int) 1 << 31;// File limit for 2GB
...
{
	cout	<<" Fastest CT Image reconstruction is in progress.... " <<endl;
	if (argc != 4)
	{
...
//	std::string module_file  = argv[1];
	...
	const unsigned int problem_size = atoi(argv[3]);
...
////////////////////////////////////////////////////////////////
	...
	unsigned int num_proj_buffer_subsets = 4; // load N_P / n projections within one buffering iteration; 4 is a safe value even for large volumes
	unsigned int num_proj_buffers = num_projections / num_proj_buffer_subsets; // 496 / 4 = 124

	const unsigned int numel_img = rctheader.img_sz[0] * rctheader.img_sz[1];//1248*960
...
//       fill the global data for the module
	...
	rctgdata.f_L = NULL;
	rctgdata.S_x = rctheader.img_sz[0];//1248
	rctgdata.S_y = rctheader.img_sz[1];//960
...
////////////////////////////////////////////////////////////////
// BEGIN ALLOC_MXMEM					// This is A[N], the projection matrix!!!
...
//       initialize the module
	if (!RCTLoadAlgorithm(&rctgdata)) // LOADALGORITHM
	{
		cerr << "Module returned error at execution of "  << endl;
		cleanup(rctgdata, projBuffers_managedByModule, out_volume_managedByModule);
		return 1;
	}
// END LOAD_MODULE
...
//       actual backprojection loop
	...
	{
		const unsigned int numProjsInIteration = min(numRemainingProjs, num_proj_buffers);// 124 or 496
...
//#endif
		for (unsigned int i=0; i<numProjsInIteration; i+=2)
		{
			...
			rctgdata.I_n_2 = rctgdata.adv_pProjBuffers[i+1];
			rctgdata.A_n_2 = pProjMatrixBuffers[cur_proj+1];
			...
			const bool ret = RCTAlgorithmBackprojection(&rctgdata); // BACKPROJECT 
			...
			numProcessedProjs=numProcessedProjs+2;
#if ! RABBIT_PROGRESS
			cout << ".";
			if ((i+1) % 62 == 0)
				cout << endl;
		...
		cout << endl;
#endif
		numRemainingProjs -= numProjsInIteration;// 496-124=372,->248 and so on!
	} // while
 //pthread_exit(NULL);
 //pthread_t thread[8*496];
 //pthread_t thread[8];
//for(int g=0;g<8;g++)
...
//       @depends BP_LOOP
	bool ret = RCTFinishAlgorithm(&rctgdata);
	if (!ret)
	{
		cerr << "Module returned error at execution of "  << endl;
		cleanup(rctgdata, projBuffers_managedByModule, out_volume_managedByModule);
...
//       @depends CALC_ERR, CALC_AVG_TIME
	...
	//cout<<"The clock ticks for the function: "<< clk_sum << endl;
// END OUTPUT_METRICS
////////////////////////////////////////////////////////////////
